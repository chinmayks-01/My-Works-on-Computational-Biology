    ...
    if not sequence:
        ...
        if seq_type:
            ...
            if protein_seq:
                ...
                st.markdown(
                    f"""
                    <div style="background: rgba(255, 255, 255, 0.02); backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; padding: 24px; text-align: center;">
                        <h3 style="color: #38bdf8; margin-top: 0; margin-bottom: 10px;">Homology Modeling</h3>
                        <p style="color: #cbd5e1; font-size: 0.95rem; margin-bottom: 20px;">
                            Click below to securely transfer your translated protein sequence ({len(protein_seq)} aa) to SWISS-MODEL for automated 3D structure prediction.
                        </p>
                        <a href="{swiss_url}" target="_blank" style="display: inline-block; background: linear-gradient(135deg, #0284c7 0%, #2563eb 100%); color: #ffffff; padding: 0.8rem 2.2rem; border-radius: 8px; font-weight: 600; text-decoration: none; box-shadow: 0 4px 15px rgba(37, 99, 235, 0.3); transition: all 0.3s ease;">
                            🚀 Open in SWISS-MODEL
                        </a>
                    </div>
                    """,
